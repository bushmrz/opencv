...
def canny_alg(path, standard_deviation, kernel_size, bound_path):
    global i
    i += 1

    img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)

    cv2.imshow('Canny from lib', cv2.Canny(img, 100, 200))

    imgBlur_CV2 = cv2.GaussianBlur(img, (kernel_size, kernel_size), standard_deviation)
    cv2.imshow('BLUR', imgBlur_CV2)


    # вычисление и вывод матрицы значений длин и матрицы значений углов градиентов
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            img_gradient_to_print[i][j] = (float(
                matr_gradient[i][j]) / max_gradient) * 255  # необходимо для корректного отображения на экране
    cv2.imshow('GRADIENT', img_gradient_to_print)
    ...
    cv2.imshow('ANGLES', img_angles_to_print)
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            ...
            # определение смещения по осям в зависимости от значения угла градиента
                ...
                is_max = gradient >= matr_gradient[i + y_shift][j + x_shift] and \
        ...
        for j in range(img.shape[1]):
            ...
            if img_border[i][j] == 255:
                ...
                # если значение градиента выше - верхней границы, то пиксель точно граница
                    ...
                    double_filtration[i][j] = 255
    cv2.imshow('DOUBLE FILTER ', double_filtration)

    cv2.waitKey(0)


# canny_alg('compcat.jpg', 3, 5, 5)
# canny_alg('compcat.jpg', 6, 15, 10)
canny_alg('compcat.jpg', 50, 9, 15)
